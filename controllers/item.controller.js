...
module.exports.addItem = async (req, res) => {
  try {
    ...
    if(isexistingItem){
      return res.status(404).json({message:"An Item with data is already exist"})
    }
...
module.exports.getAllItems=async (req,res)=>{
  try{
    ...
    res.status(200).json({message:"Items Fetched Successfully" , data:items , hasMore:hasMore , total:total})
...
module.exports.updateItem=async (req,res)=>{
  try{
    const {id}=req.params
    const { name, category, price, model, brand, stock ,categoryName , brandName , modelName } = req.body;
    if(!id){
      return res.status(404).json({message:"Item Id is required"})
    }
    const parsedId=parseInt(id)
